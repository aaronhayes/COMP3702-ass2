...
/**
 * <p>A graphical panel capable of displaying and editing a Bitmap.</p>
 * @author Mikael Bodén
 * @version 1.0
 */
...
public class BitmapPanel extends JPanel {
   ...
   * @param map The bitmap to display and edit.
   */
  ...
  /**
   * Edit the bitmap by inserting a blob (with a specified width) at a specified position.
   * @param xx The x position of the edit
   * @param yy The y position of the edit
   * @param thickness The thickness of the brush to be used
   */
  public void brush(int xx, int yy, int thickness) {
    int posX=Math.max(Math.min((xx-marginX)/dx,map.getCols()-1),0);
    int posY=Math.max(Math.min((yy-marginY)/dy,map.getRows()-1),0);
  ...
  public void paintComponent(Graphics g) {
    ...
    for (int x=1; x<map.getCols(); x++) {
      g2d.drawLine(marginX+x*dx, marginY, marginX+x*dx, marginY+map.getRows()*dy);
    }
    for (int y=1; y<map.getRows(); y++) {
      g2d.drawLine(marginX, marginY+y*dy, marginX+map.getCols()*dx, marginY+y*dy);
    }
